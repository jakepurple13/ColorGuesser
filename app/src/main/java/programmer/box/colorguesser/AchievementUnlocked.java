...
public class AchievementUnlocked {
    ...
    public AchievementUnlocked(Activity activity) {
    ...
    private void initGlobalFields() {
        if (!initiatedGlobalFields) {
            ...
            achievementLayout = new RelativeLayout(activity);
            ...
            LinearLayout textContainerFake = new LinearLayout(activity);
            ...
            TextView titleFake = new TextView(activity);
            ...
            ScrollTextView subtitleFake = new ScrollTextView(activity);
            ...
            LinearLayout achievementIconBg = new LinearLayout(activity);
            ...
            icon = new AchievementIconView(activity);
            int padding = 7;
            icon.setPadding(convertDpToPixel(padding), convertDpToPixel(padding), convertDpToPixel(padding), convertDpToPixel(padding));
            //LayoutParams achievementIconLP = new LayoutParams(largeSize, largeSize);
            LayoutParams achievementIconLP = new LayoutParams(ViewGroup.LayoutParams.MATCH_PARENT, ViewGroup.LayoutParams.MATCH_PARENT);
            ...
            subtitleTextView = new ScrollTextView(activity);
    ...
    private void buildAchievement() {
        ...
        //icon.setBackground(activity.getDrawable(R.drawable.border_corners));
        //achievementLayout.findViewWithTag("achievementIconBg").setBackground(activity.getDrawable(R.drawable.border_corners));
        ...
        if (alignTop) {
            mainViewLP.gravity = Gravity.TOP | Gravity.END;
        } else {
        ...
        activity.addContentView(achievementLayout, mainViewLP);
        added = true;
    }
    ...
    private void removeView() {
        ...
        try {
            ...
            achievementLayout.removeView(achievementLayout);
...
//        } else sets.play(animateData(achievements[index]));
        ...
        result.addListener(new AnimatorListenerAdapter() {
            ...
            public void onAnimationStart(Animator animation) {
                ...
                if (added) {
                    ...
                    //((WindowManager) activity.getSystemService(Context.WINDOW_SERVICE)).updateViewLayout(achievementLayout, mainViewLP);
                    //((WindowManager) activity.getApplicationContext().getSystemService(Context.WINDOW_SERVICE)).updateViewLayout(achievementLayout, mainViewLP);
                }
    ...
    private void setIcon(AchievementData data) {
        ...
        if (data.getIconColor() != 0) {
            UtilImage.changeDrawableColor(d, data.getIconColor());
        }
        if (d != null) {
...
class AchievementData {
    ...
    private int textColor = 0xff000000, backgroundColor = 0xffffffff, iconBackgroundColor = 0x0, iconColor = 0x0;
    ...
    AchievementData setIconColor(int iconColor) {
    ...
    public void setDrawable(final Drawable drawable) {
        ...
        if (getDrawable() == null) {
            ...
            animate().scaleX(0f).setDuration(200 * AchievementUnlocked.animationMultiplier).scaleY(0f).alpha(0f).withEndAction(new Runnable() {
                @Override
                public void run() {
                    animate().setDuration(200 * AchievementUnlocked.animationMultiplier).alpha(1f).withStartAction(new Runnable() {
                        @Override
                        public void run() {
                            setImageDrawable(drawable);
                        }
